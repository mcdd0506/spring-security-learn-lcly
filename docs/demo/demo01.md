...
Spring Security 为 [授权](https://en.wikipedia.org/wiki/Authorization) 提供全面支持。**授权是指确定允许谁访问特定资源**。Spring Security 允许基于请求的授权和基于方法的授权，从而提供 [深度防御](https://en.wikipedia.org/wiki/Defense_in_depth_(computing))。
